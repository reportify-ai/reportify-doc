...
|------|------|
| `RSI(14) < 30` | RSI 超卖 |
| `CROSS(MA(5), MA(10))` | 均线金叉（MA5 上穿 MA10） |
| `CROSSDOWN(MA(5), MA(10))` | 均线死叉（MA5 下穿 MA10） |
| `(CLOSE > MA(20)) & (MA(20) > MA(60))` | 多头趋势 |
| `CLOSE > BOLL(20, 2).upper` | 突破布林带上轨 |
| `(KDJ().j < 20) & CROSS(KDJ().k, KDJ().d)` | KDJ 低位金叉 |
| `MACD().dif > MACD().dea` | MACD 多头 |
| `CLOSE > REF(CLOSE, 1) * 1.05` | 较昨日涨幅超 5% |
| `CLOSE > HHV(20)` | 创 20 日新高 |
| `COUNT(CLOSE > REF(CLOSE, 1), 5) >= 4` | 近 5 日至少 4 天上涨 |
| `EVERY(CLOSE > REF(CLOSE, 1), 3)` | 连续 3 天上涨 |
| `(CLOSE > MA(10)) & (MA(10) > MA(20)) & (MA(20) > MA(60))` | 多均线多头排列 |
| `(MA(10) > REF(MA(10), 5)) & (MA(20) > REF(MA(20), 5))` | 均线向上发散 |
| `VOLUME > MA(20, VOLUME) * 2` | 成交量放大（2 倍于 20 日均量） |
...
| `(ROE > 0.12) & (CLOSE > MA(20))` | 质量 + 上涨趋势 |
| `(PB < 1.5) & CROSS(MA(5), MA(20))` | 价值 + 金叉 |
