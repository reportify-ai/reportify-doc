---
title: '获取 1 分钟 K 线数据'
description: '获取单只股票的 1 分钟 K 线数据'
openapi: 'POST /v1/quant/quotes/1mkline'
---

获取单只股票的 1 分钟 K 线（开高低收量）数据。

<CodeGroup>

```bash cURL
curl -X POST https://api.reportify.cn/v1/quant/quotes/1mkline \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{
    "symbol": "000001",
    "start_datetime": "2024-01-01 09:30:00",
    "end_datetime": "2024-01-01 15:00:00",
    "market": "cn"
  }'
```

```python Python
import requests

response = requests.post(
    "https://api.reportify.cn/v1/quant/quotes/1mkline",
    headers={"Authorization": "Bearer <token>"},
    json={
        "symbol": "000001",
        "start_datetime": "2024-01-01 09:30:00",
        "end_datetime": "2024-01-01 15:00:00",
        "market": "cn"
    }
)
print(response.json())
```

```typescript TypeScript
const response = await fetch('https://api.reportify.cn/v1/quant/quotes/1mkline', {
  method: 'POST',
  headers: {
    'Authorization': 'Bearer <token>',
    'Content-Type': 'application/json'
  },
  body: JSON.stringify({
    symbol: '000001',
    start_datetime: '2024-01-01 09:30:00',
    end_datetime: '2024-01-01 15:00:00',
    market: 'cn'
  })
});
const data = await response.json();
```

</CodeGroup>

## 请求参数

<ParamField body="symbol" type="string" required>
  股票代码
</ParamField>

<ParamField body="start_datetime" type="string" required>
  开始时间，格式：`YYYY-MM-DD HH:MM:SS`
</ParamField>

<ParamField body="end_datetime" type="string" required>
  结束时间，格式：`YYYY-MM-DD HH:MM:SS`
</ParamField>

<ParamField body="market" type="string" default="cn">
  股票市场：`cn`（A股）, `hk`（港股）, `us`（美股）
</ParamField>

## 响应参数

<ResponseField name="datas" type="array">
  1 分钟 K 线数据列表
  <Expandable title="数据字段">
    <ResponseField name="datetime" type="string">
      日期时间
    </ResponseField>
    <ResponseField name="symbol" type="string">
      股票代码
    </ResponseField>
    <ResponseField name="open" type="number">
      开盘价
    </ResponseField>
    <ResponseField name="high" type="number">
      最高价
    </ResponseField>
    <ResponseField name="low" type="number">
      最低价
    </ResponseField>
    <ResponseField name="close" type="number">
      收盘价
    </ResponseField>
    <ResponseField name="volume" type="integer">
      成交量
    </ResponseField>
  </Expandable>
</ResponseField>

<ResponseField name="metadata" type="object">
  查询元数据
</ResponseField>

## 响应示例

```json
{
  "datas": [
    {
      "datetime": "2024-01-01 09:30:00",
      "symbol": "000001",
      "open": 12.15,
      "high": 12.20,
      "low": 12.10,
      "close": 12.18,
      "volume": 1234567
    },
    {
      "datetime": "2024-01-01 09:31:00",
      "symbol": "000001",
      "open": 12.18,
      "high": 12.25,
      "low": 12.15,
      "close": 12.22,
      "volume": 2345678
    },
    {
      "datetime": "2024-01-01 09:32:00",
      "symbol": "000001",
      "open": 12.22,
      "high": 12.28,
      "low": 12.20,
      "close": 12.25,
      "volume": 3456789
    }
  ],
  "metadata": {
    "start_datetime": "2024-01-01 09:30:00",
    "end_datetime": "2024-01-01 15:00:00",
    "count": 3
  }
}
```

## 使用示例

### 获取当日交易数据

```python
import requests
from datetime import datetime, timedelta

# 获取当日 9:30 到 15:00 的 1 分钟 K 线
today = datetime.now().strftime("%Y-%m-%d")
response = requests.post(
    "https://api.reportify.cn/v1/quant/quotes/1mkline",
    headers={"Authorization": "Bearer <token>"},
    json={
        "symbol": "600519",
        "start_datetime": f"{today} 09:30:00",
        "end_datetime": f"{today} 15:00:00",
        "market": "cn"
    }
)
data = response.json()

for row in data["datas"][:10]:
    print(f"{row['datetime']}: 开{row['open']} 高{row['high']} 低{row['low']} 收{row['close']} 量{row['volume']}")
```

### 获取港股数据

```python
response = requests.post(
    "https://api.reportify.cn/v1/quant/quotes/1mkline",
    headers={"Authorization": "Bearer <token>"},
    json={
        "symbol": "00700",
        "start_datetime": "2024-01-01 09:30:00",
        "end_datetime": "2024-01-01 16:00:00",
        "market": "hk"
    }
)
```

### 获取美股数据

```python
response = requests.post(
    "https://api.reportify.cn/v1/quant/quotes/1mkline",
    headers={"Authorization": "Bearer <token>"},
    json={
        "symbol": "AAPL",
        "start_datetime": "2024-01-01 09:30:00",
        "end_datetime": "2024-01-01 16:00:00",
        "market": "us"
    }
)
```
