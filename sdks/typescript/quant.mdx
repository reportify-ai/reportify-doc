...
| `positionSize` | `number` | 否 | 0.2 | 单只股票最大仓位比例（占总资产） |
| `maxPositions` | `number` | 否 | 5 | 最大持仓股票数量 |
| `minVolume` | `number` | 否 | 100 | 最小买入股数（如A股1手） |
| `autoClose` | `boolean` | 否 | true | 是否自动平仓 |
| `indicator` | `Record<string, string>` | 否 | - | 指标字典，用于策略计算或返回 |
