...
data.forEach(d => {
  console.log(`${d.report_period}: 营收${d.revenue} 净利润${d.net_profit}`);
});
```
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `financialType` | `string` | 是 | 报表类型（见下表） |
| `symbol` | `string` | 是 | 股票代码 |
| `market` | `string` | 否 | 市场，默认 cn |
| `startYear` | `number` | 否 | 开始年份，默认 3 年前 |
| `endYear` | `number` | 否 | 结束年份，默认当前年份 |
...
```typescript
// 获取利润表
const income = await client.quant.financials({
  financialType: 'income',
  symbol: '600519'
});

// 获取资产负债表
const balance = await client.quant.financials({
...
// 获取港股现金流量表
const cashflow = await client.quant.financials({
...
// 获取美股财务数据
const usIncome = await client.quant.financials({
  financialType: 'income',
  symbol: 'AAPL',
  market: 'us'
});
...
const result = await client.quant.backtest({
  ...
  symbol: '000001',
  market: 'cn',
  entryFormula: 'CROSS(MA(5), MA(20))',  // 买入信号：MA5 上穿 MA20
  exitFormula: 'CROSSDOWN(MA(5), MA(20))',  // 卖出信号（可选）
...
const result2 = await client.quant.backtest({
  ...
  endDate: '2024-01-01',
  symbol: '000001',
  entryFormula: 'CROSS(MA(5), MA(20))',      // 金叉买入
  exitFormula: 'CROSSDOWN(MA(5), MA(20))'    // 死叉卖出
});
