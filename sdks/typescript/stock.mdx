...
```typescript
// 通过股票代码查询（支持多个，逗号分隔）
const info = await client.stock.overview({ symbols: 'US:AAPL,HK:00700' });
console.log(info);
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `options.symbols` | `string` | 否 | 股票代码，支持多个，用逗号分隔，如 `US:AAPL,HK:00700` |
...
// 获取流通股股东
const outstandingShareholders = await client.stock.shareholders('US:AAPL', {
...
| `options.fiscalQuarter` | `string` | 否 | - | 指定财季（Q1, Q2, Q3, Q4, FY, H1） |
...
// 带参数查询
const cashflow2 = await client.stock.cashflowStatement('US:AAPL', {
  period: 'quarterly',
  limit: 8,
  calendar: 'fiscal'
});
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `symbol` | `string` | 是 | 股票代码，如 `US:AAPL`、`688001`、`00700` |
...
| `options.symbol` | `string` | 否 | 股票代码，如 `US:AAPL` |
