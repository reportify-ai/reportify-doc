...
# 复合条件
df = client.quant.compute_factors(["000001"], "(CLOSE > MA(20)) & (MA(20) > MA(60))")
```
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `financial_type` | `str` | 是 | 报表类型（见下表） |
| `symbol` | `str` | 是 | 股票代码 |
| `market` | `str` | 否 | 市场，默认 cn |
| `start_year` | `int` | 否 | 开始年份，默认 3 年前 |
| `end_year` | `int` | 否 | 结束年份，默认当前年份 |
...
```python
# 获取利润表
df = client.quant.financials("income", "600519")
for _, row in df.iterrows():
    print(f"{row['report_period']}: 营收{row['revenue']/1e8:.2f}亿 净利润{row['net_profit']/1e8:.2f}亿")

# 获取资产负债表
df = client.quant.financials("balancesheet", "000001", start_year=2020, end_year=2024)

# 获取港股现金流量表
df = client.quant.financials("cashflow", "00700", market="hk")

# 获取美股财务数据
df = client.quant.financials("income", "AAPL", market="us")
```
...
result = client.quant.backtest(
    ...
    symbol="000001",
    market="cn",
    entry_formula="CROSS(MA(5), MA(20))",  # 买入信号：MA5 上穿 MA20
    exit_formula="CROSSDOWN(MA(5), MA(20))",  # 卖出信号（可选）
    initial_cash=100000,  # 初始资金
    commission=0.0003,    # 佣金费率
    stop_loss=0.05,       # 止损 5%
    sizer_percent=99,     # 仓位 99%
    auto_close=True,      # 自动平仓
    labels={"rsi": "RSI(14)", "ma20": "MA(20)"}  # 额外指标（可选）
)
```
...
result = client.quant.backtest(
    ...
    end_date="2024-01-01",
    symbol="000001",
    entry_formula="CROSS(MA(5), MA(20))",      # 金叉买入
    exit_formula="CROSSDOWN(MA(5), MA(20))"    # 死叉卖出
)
