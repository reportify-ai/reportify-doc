...
| `strategy_code` | `str` | 否 | None | 自定义策略代码 |
...
| `position_size` | `float` | 否 | 0.2 | 单只股票最大仓位比例（占总资产） |
| `max_positions` | `int` | 否 | 5 | 最大持仓股票数量 |
| `min_volume` | `int` | 否 | 100 | 最小买入股数（如A股1手） |
| `auto_close` | `bool` | 否 | True | 是否自动平仓 |
| `indicator` | `dict[str, str]` | 否 | None | 指标字典，用于策略计算或返回 |
