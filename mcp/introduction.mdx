...
<AccordionGroup>
  ...
  | `earnings_search` | Search for earnings-related documents submitted to exchanges (query optional, sorted by date desc or relevance) |
  | `conferences_search` | Search for earnings-related conference call transcripts and presentation slides (query optional, sorted by date desc or relevance) |
...
<AccordionGroup>
  ...
  **公共参数**：
  ...
  | 工具名 | 功能描述 |
  |--------|----------|
  | `factors` | 获取可用因子列表（含变量、函数、技术指标、基本面因子） |
  | `factors_compute` | 计算因子值（RSI, MACD, KDJ, BOLL, ATR 等技术指标也在其中） |
  | `factors_screen` | 因子筛选选股 |
  | `ohlcv` | 获取单只股票 OHLCV 数据 |
  | `ohlcv_batch` | 批量获取多只股票 OHLCV 数据 |
  | `kline` | 获取 K 线数据（支持 1M/5M/15M/30M/60M/1D/1W/1MO） |
  | `kline_batch` | 批量获取多只股票 K 线 |
  | `backtest` | 策略回测 |
  
  **支持的因子类型**：
  - 技术因子：`RSI(14)`, `MACD()`, `BOLL(20,2)`, `KDJ()`, `ATR(14)` 等
  - 基本面因子：`PE()`, `PE_TTM()`, `PB()`, `ROE()`, `ROA()`, `EPS()` 等
  - 财务报表：`INCOME.net_profit`, `BALANCESHEET.total_assets` 等
...
<Tabs>
  ...
  <Tab title="计算指标">
    使用 Stock MCP 计算 RSI 指标：

    ```json
    {
      "tool": "factors_compute",
      "parameters": {
        "market": "cn",
