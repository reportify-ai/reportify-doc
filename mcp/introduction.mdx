...
<AccordionGroup>
  ...
  **公共参数**：
  ...
  | 工具名 | 功能描述 |
  |--------|----------|
  | `indicators` | 获取可用技术指标列表 |
  | `indicators_compute` | 计算技术指标（RSI, MACD, KDJ, BOLL, ATR 等） |
  | `factors` | 获取可用因子列表（技术因子 + 基本面因子） |
  | `factors_compute` | 计算因子值（支持技术和基本面因子） |
